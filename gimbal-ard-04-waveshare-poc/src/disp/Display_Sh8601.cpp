...
esp_err_t Display_Sh8601::displayNewPanel(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t* panel_dev_config, esp_lcd_panel_handle_t* ret_panel) {
    ...
    sh8601->base.del = displayDel;
    sh8601->base.reset = displayReset;
    sh8601->base.init = displayInit;
    sh8601->base.draw_bitmap = displayDrawBitmap;
    sh8601->base.invert_color = displayInvertColor;
    sh8601->base.set_gap = displaySetGap;
    sh8601->base.mirror = displayMirror;
    sh8601->base.swap_xy = displaySwapXY;
...
esp_err_t Display_Sh8601::tx_param(sh8601_panel_t* sh8601, esp_lcd_panel_io_handle_t io, int lcd_cmd, const void* param, size_t param_size) {
...
esp_err_t Display_Sh8601::tx_color(sh8601_panel_t* sh8601, esp_lcd_panel_io_handle_t io, int lcd_cmd, const void* param, size_t param_size) {
    if (sh8601->flags.use_qspi_interface) {
        lcd_cmd &= 0xff;
        lcd_cmd <<= 8;
...
esp_err_t Display_Sh8601::displayDel(esp_lcd_panel_t* panel) {
...
esp_err_t Display_Sh8601::displayReset(esp_lcd_panel_t* panel) {
    sh8601_panel_t* sh8601 = __containerof(panel, sh8601_panel_t, base);
...
esp_err_t Display_Sh8601::displayInit(esp_lcd_panel_t* panel) {
    ...
    uint8_t madctl_arr[1] = { sh8601->madctl_val };
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_MADCTL, madctl_arr, 1), TAG, "send command failed");
    uint8_t colmod_arr[1] = { sh8601->colmod_val };
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_COLMOD, colmod_arr, 1), TAG, "send command failed");
...
esp_err_t Display_Sh8601::displayDrawBitmap(esp_lcd_panel_t* panel, int x_start, int y_start, int x_end, int y_end, const void* color_data) {
    ...
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_CASET, x_arr, 4), TAG, "send command failed");
    // define an area of frame memory where MCU can access
    uint8_t y_arr[4] = {
    ...
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_RASET, y_arr, 4), TAG, "send command failed");
...
esp_err_t Display_Sh8601::displayInvertColor(esp_lcd_panel_t* panel, bool invert_color_data) {
    sh8601_panel_t* sh8601 = __containerof(panel, sh8601_panel_t, base);
    esp_lcd_panel_io_handle_t io = sh8601->io;
    int command = 0;
...
esp_err_t Display_Sh8601::displayMirror(esp_lcd_panel_t* panel, bool mirror_x, bool mirror_y) {
...
esp_err_t Display_Sh8601::displaySwapXY(esp_lcd_panel_t* panel, bool swap_axes) {
    ESP_LOGE(TAG, "swap_xy is not supported by this panel");
    return ESP_ERR_NOT_SUPPORTED;
}

esp_err_t Display_Sh8601::displaySetGap(esp_lcd_panel_t* panel, int x_gap, int y_gap) {
    sh8601_panel_t* sh8601 = __containerof(panel, sh8601_panel_t, base);
    sh8601->x_gap = x_gap;
    sh8601->y_gap = y_gap;
    return ESP_OK;
